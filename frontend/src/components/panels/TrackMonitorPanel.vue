...
const activeTracks = computed(() => {
  const last = alertStore.todayAlerts.find(a => a.rawStatus === 'PROCESSING')
  return last?.trackCount || 0
})

const events = computed(() =>
  alertStore.todayAlerts.flatMap(a =>
    (a.subAlerts || []).map((s, i) => ({
      id: `${a.id}-${i}`,
      type: s.alertType,
...
.ts-item { flex: 1; background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: 6px; padding: 8px; display: flex; flex-direction: column; align-items: center; gap: 3px; }
.ts-num { font-size: 22px; font-weight: 700; color: var(--text-secondary); line-height: 1; }
.ts-num.active { color: var(--color-primary); }
.ts-label { font-size: 10px; color: var(--text-muted); }

.event-stream { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
.stream-title { font-size: 11px; color: var(--text-dim); margin-bottom: 5px; }
.stream-list { flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 4px; }

.stream-item { display: flex; align-items: center; gap: 6px; font-size: 11px; padding: 4px 6px; border-radius: 4px; background: var(--bg-event-item); border-left: 2px solid var(--text-dim); }
.stream-item.danger { border-left-color: var(--color-danger); background: var(--bg-event-danger); }
.stream-item.warn { border-left-color: var(--color-warning); background: var(--bg-event-warning); }

.event-dot { width: 5px; height: 5px; border-radius: 50%; background: currentColor; flex-shrink: 0; }
.event-type { color: var(--text-secondary); font-size: 10px; width: 72px; flex-shrink: 0; overflow: hidden; }
.event-desc { flex: 1; color: var(--text-muted); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.event-time { color: var(--text-dark); font-size: 10px; flex-shrink: 0; }

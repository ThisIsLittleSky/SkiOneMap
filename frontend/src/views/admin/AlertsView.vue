  ...
  <div class="page-body">
    ...
    <div v-if="tab === 'alerts'">
      ...
      <div v-else class="alert-cards">
        <div
          v-for="(alert, i) in alertStore.alerts" :key="i"
          class="alert-card"
        ...
        >
          ...
          <div class="alert-card-body">
            ...
            <div v-if="alert.subAlerts && alert.subAlerts.length > 0" class="sub-alerts">
              ...
              <ul>
                <li v-for="(sa, si) in alert.subAlerts" :key="si">[{{ sa.alertType }}] {{ sa.description }}</li>
              </ul>
            </div>
    ...
    <div v-if="tab === 'tasks'">
      ...
      <table v-else>
        <thead>
          <tr><th>任务ID</th><th>视频ID</th><th>状态</th><th>创建时间</th><th>操作</th></tr>
        </thead>
        <tbody>
      ...
      <div v-if="taskDetail" class="task-detail">
        ...
        <div class="detail-body">
          ...
          <div v-if="taskDetail.alerts.length > 0" class="detail-alerts">
            <strong>预警明细：</strong>
            <ul>
              <li v-for="a in taskDetail.alerts" :key="a.id">[{{ a.alertType }}] {{ a.description }} ({{ a.severity }})</li>
            </ul>
          </div>
        </div>
      </div>
...
async function loadTasks() {
  ...
  try { const res = await listTasks(); tasks.value = res.data }
...
function taskStatusText(s: string) {
  return ({ PENDING: '待处理', PROCESSING: '分析中', COMPLETED: '已完成', FAILED: '失败' } as Record<string, string>)[s] || s
}
